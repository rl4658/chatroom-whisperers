...
// Define mutexes for protecting critical sections
std::mutex roomsMutex;
std::mutex roomDataMutex;
...
std::vector<char> getAllChatroomDataAsByteArray(const std::vector<ChatRoomStructure> &rooms)
{
    ...
    // Raymond
    // Lock the mutex before accessing room_data vector ******************* MUTEX HERE
...
{
    // Raymond
    // Lock the mutex before accessing/modifying connectedClients vector
    std::lock_guard<std::mutex> lock(connectedClientsMutex);
    std::lock_guard<std::mutex> roomsLock(roomsMutex); // Lock roomsMutex
    // roomsLock guards the roomsMutex. This mutex is used to synchronize access to the rooms vector
    // or any data structure related to it, ensuring thread safety when modifying it.
...
{
    // Raymond
    //  Lock the mutex before accessing connectedClients vector
    std::lock_guard<std::mutex> lock(connectedClientsMutex);
    ...
    virtual long ThreadMain()
    {
        ...
        while (this->isActive)
        {
            try
            {
                ...
                // Lock the mutex before accessing connectedClients vector
                std::lock_guard<std::mutex> lockConnectedClients(connectedClientsMutex);

                connectedClients.push_back(&socketRef);

                // Lock the mutex before accessing chatroomData
                std::lock_guard<std::mutex> lockRoomData(roomDataMutex);
